...
/* ---------- Employee ↔ Cognito + Corp Check (mismatches only) ---------- */

/** Similar columns are kept together, always in auth / cognito / corp order. */
const EMP_COGNITO_HEADERS = [
  ...
  "Short Code (Cognito)",
  "Short Code (Corp)",
  "Short Code Mismatch (Auth vs Cognito)",
  "Name (Auth)",
  "Name (Cognito)",
  "Name (Corp)",
  "Name Mismatch (Auth vs Corp)",
  "Mobile (Auth)",
  "Mobile (Cognito)",
  "Mobile (Corp)",
  "Mobile Mismatch (Auth vs Cognito)",
  "Mobile Mismatch (Auth vs Corp)",
  "Cognito ID (Auth)",
  "Cognito ID (Corp)",
...
function empCognitoRowCells(r: EmployeeCognitoRow): string[] {
  return [
    ...
    r.auth.cognito_id ?? "",
    r.corp?.cognito_id ?? "",
    yn(r.flags.corpCognitoIdMismatch),
    yn(r.cognito !== null),
    yn(r.corp !== null),
    r.cognito?.username ?? "",
    r.cognito?.status ?? "",
