...
import { fixCognitoId } from "@/lib/correction";
...
type Body = {
  ...
  empmasterId?: string;
  /** true → report what would change without writing. */
  preview?: boolean;
};
...
export async function POST(req: Request) {
  ...
  try {
    const result = await fixCognitoId(environment, empmasterId, preview);
    return NextResponse.json(result);
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    console.error(`[correction/fix-cognito ${environment} ${empmasterId}]`, msg);
