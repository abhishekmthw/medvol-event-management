...
| `lib/cognito.ts` | Cognito client (`@aws-sdk/client-cognito-identity-provider`), cached per env. Reads: `lookupByMobile` (filter `phone_number = "+91…"`) and `lookupBySub` (filter `sub = "…"`) via `ListUsers`; parses `sub`, `name`, `phone_number`, `custom:emp_short_code`, `custom:ucode`, username/status/enabled. ONE write path: `updateUserAttributes` (`AdminUpdateUserAttributes`; `updateUserPhone` wraps it for phone_number + verified) — the tool only ever writes `phone_number` to Cognito (identity attributes like `custom:emp_short_code` / `name` / `custom:ucode` are NEVER rewritten), used only by the Data Correction card's confirmed repair action. Reuses `{ENV}_AWS_*` creds + `{ENV}_COGNITO_USERPOOL_ID`. |
...
- **AWS Cognito** (direct, via `@aws-sdk/client-cognito-identity-provider`) — `ListUsers` on the field-force user pool (`{ENV}_COGNITO_USERPOOL_ID`), filtered by `phone_number` or `sub`, plus ONE write path: `AdminUpdateUserAttributes` setting `phone_number` (+ verified) ONLY — Cognito identity attributes (`custom:emp_short_code` / `name` / `custom:ucode`) are never rewritten; used only by the Data Correction card's confirmed repair action. Credentials/region reuse the per-env `{ENV}_AWS_*` vars; those IAM creds need `cognito-idp:ListUsers` + `cognito-idp:AdminUpdateUserAttributes`.
