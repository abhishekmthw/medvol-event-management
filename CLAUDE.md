...
| `lib/auth-comparison.ts` | Corp-driven Auth/Corp/Cognito reconciliation (READ-ONLY). `empmaster_hdr` (corp pool) is the base/truth; each corp employee is matched to `Field_Force_Users` (auth pool) by (short code, company code) and validated against Cognito (the truth for `cognito_id`). Corp `active_status='Y'` scope filter; auth fetched unfiltered. `normalizeMobile`, `compareByMobile`, `scanInconsistent`, `isEmployeeScope`. |
