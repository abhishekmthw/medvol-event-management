...
- **Tables read/written:**
  ...
  - **Employee Data Correction (writes on confirm):** additionally reads `u_code` (corp) / `ucode` (auth) and corp `public.events` by `"eventStreamStreamId" = 'employee_<empmaster_id>'` (for replay). On confirmed fixes it UPDATEs `public.empmaster_hdr.cognito_id` (corp DB) and `"Field_Force_Users".cognito_id` (auth DB) — single-column, single-row, parameterized.
