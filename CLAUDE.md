...
| `app/api/auth-comparison/correction/sync-cognito-attrs/route.ts` | `POST {environment, empmasterId, preview}` — overwrites the Cognito user's stale `custom:emp_short_code` / `name` / `custom:ucode` from corp when the account holds the corp mobile but identifies a different employee; other corp/auth records storing the same sub have their `cognito_id` NULLed in the same run (`preview: true` = attribute before/after + released-from + claimed-by records only). |
...
7. **Sync Cognito details from corp** — the MIRROR of #6: the stored sub's Cognito owner HOLDS the corp mobile but its `custom:emp_short_code` / `name` / `custom:ucode` identify a different employee (e.g. a reassigned number whose Cognito profile was never updated). The corp/auth linkage + matching phone are two independent identity signals, so the account is treated as this employee's and the stale attributes are overwritten from corp (`custom:emp_short_code`, `name`, `custom:ucode` uppercase — Cognito's convention; only the attributes that differ) via `updateUserAttributes`. A phone-matching owner is never treated as "wrong"/clearable by the stored-sub audit — this action is offered instead of Clear. Guards: refused on lookup errors or when a live mobile+short-code target exists (fix applies). OTHER corp/auth records storing the same sub do **NOT** block — corp is the source of truth and the account holds this employee's corp mobile, so they are stale claimants: the preview lists them (`releasedFrom`) and the run NULLs their `cognito_id` FIRST via `releaseSubFromRecords` (shared with action #4; conditional `AND cognito_id = <sub>`), then writes the Cognito attributes — that order leaves a cleanly re-runnable state if the Cognito write fails. Analyze the released employees' mobiles afterwards to relink them to their own accounts. The preview also lists corp employees carrying the account's CURRENT (stale) short code ("who it claims to be") so the operator verifies that identity moved off the number before confirming.
