...
- Outbound calls to the external **Playground SQS API** to `deleteMessage` / `changeMessageVisibility` on SQS receipt handles.
...
| `lib/instances.ts` | Reads `PRIVATE_INSTANCES` (comma-sep ids) and per-instance `_LABEL` / `_SERVICE` / `_PLAYGROUND_FLAG` env vars. Defaults `playgroundFlag` to `is{Id}` (e.g., `isLupin`). |
...
| `lib/playground.ts` | HTTPS client for the Playground SQS API (POST `{action, handles, isOms?, is{Instance}?}`, expects body `"success"` or `"failed"` → maps to `success` / `gone`) and the Playground batch scheduler API (DELETE `{scheduler_name}`). Honors `PLAYGROUND_FETCH_TIMEOUT_MS` (default 10s) with `AbortController`. |
...
| `clear-by-event-ids` | failed `event_consumer_status` rows for the given IDs | `event_status = 'Success'`, `"forceStatus" = true`; calls Playground SQS `delete` on each receipt handle | If a row has no receipt handle, DB is still force-succeeded ("gone"). A 15-day-old receipt also returns "gone". |
| `refire-by-event-ids` | failed `event_consumer_status` rows for the given IDs | none in DB (by spec); calls Playground SQS `changeVisibility` to re-deliver | If the receipt is "gone" (>15 day SQS retention) the row reports an error suggesting `clear-by-event-ids` instead. **Never touches the DB.** |
...
**Preview vs run.** All destructive actions accept `{preview: true}` in the request. Preview executes only the SELECT (no DB writes, no Playground calls) and returns the candidate rows + count. The UI requires explicit confirmation in a modal before sending `{preview: false}`. Production targets render the confirm button in danger red.
...
| `PLAYGROUND_FETCH_TIMEOUT_MS` | Timeout for outbound Playground HTTP calls. Default `10000`. |
| `PLAYGROUND_SQS_API_URL_PROD` | Playground SQS endpoint for prod queues. |
| `PLAYGROUND_SQS_API_URL_STAGE` | Playground SQS endpoint for stage queues. |
| `PLAYGROUND_SQS_API_KEY` | `x-api-key` header for both prod and stage SQS endpoints. |
...
| `PRIVATE_INSTANCE_{ID}_PLAYGROUND_FLAG` | JSON key the Playground SQS API expects to route to this instance. Defaults to `is{Id}` (e.g. `isLupin`). |
...
This service does **not** call AWS APIs directly — there are no AWS SDK packages installed (`grep aws-sdk` is empty) and no IAM credentials in env. SQS deletion and `changeVisibility` are delegated to the external Playground SQS API (which holds the AWS creds server-side). The only AWS-adjacent assumption is that the Playground API itself acts on queues in `ap-south-1` (the platform's default region).
...
- **Playground SQS API** — Used for: `POST {action: "delete" | "changeVisibility", handles: string[], isOms?: true, is{Instance}?: true}` against a single endpoint per environment. Response body `"success"` → operation acknowledged; `"failed"` → receipt handle no longer valid (treated as "gone"); anything else → error. Authentication via `x-api-key` header.
- **Playground batch scheduler API** — Used for: `DELETE` with `{scheduler_name}` body. Authentication via `Authorization: {"apiKey":"…"}` header (note the JSON-encoded value). Removes the EventBridge Scheduler that would have re-fired the batch.

Both APIs are operated by the same internal team; this app is one of several consumers.
...
This service does **not** publish events, does not call any other Medvol backend, and does not call AWS directly — all side effects are scoped to direct DB writes against the listed tables plus HTTP calls to the two Playground APIs.
