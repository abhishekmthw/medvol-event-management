...
- **AWS Cognito** (direct, via `@aws-sdk/client-cognito-identity-provider`) — `ListUsers` on the field-force user pool (`{ENV}_COGNITO_USERPOOL_ID`), filtered by `phone_number` or `sub`, plus ONE write: `AdminUpdateUserAttributes` setting `phone_number` (+ verified) from corp, used only by the Data Correction card's confirmed phone-fix action. Credentials/region reuse the per-env `{ENV}_AWS_*` vars; those IAM creds need `cognito-idp:ListUsers` + `cognito-idp:AdminUpdateUserAttributes`.
