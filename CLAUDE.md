...
| `app/api/auth-comparison/correction/fix-cognito-phone/route.ts` | `POST {environment, empmasterId, preview}` — sets the Cognito user's phone_number to the corp mobile (verified) when the stored sub's owner matches the corp short code but holds a different phone (`preview: true` = old/new mobile + old-mobile holders only). |
| `app/api/auth-comparison/correction/reassign-owner/route.ts` | `POST {environment, empmasterId, preview}` — returns a Cognito account to its short-code owner when it holds THIS employee's corp mobile but is labeled with another short code: fixes the account's Cognito mobile from the OWNER's corp record, writes its sub to the owner's corp/auth, and NULLs the stale link on this employee (`preview: true` = planned repairs only). |
...
5. **Clear wrong cognito_id** — offered when NO rightful Cognito user resolves for the employee (mobile lookup empty / short-code mismatch) but a cognito_id IS stored on its corp/auth row(s) and the stored-sub audit confirms it wrong (stale, or owned by a different user). The preview lists each side (source, record id, stored sub, why wrong); on confirm the wrong side(s) are NULLed with an `AND cognito_id = <sub>` predicate. Refused when a live target exists (use the fix instead), when the sub's owner matches the short code (Cognito phone drift → use the phone update action), or on any Cognito lookup error (never clear on uncertainty). The "holistic" two-user repair: clear the foreign sub here, then analyze the owner's mobile to fix THEIR records.
6. **Update Cognito mobile from corp** — offered when the stored sub's Cognito owner MATCHES the corp short code but holds a different `phone_number` (so the mobile lookup couldn't resolve it). `updateUserPhone` (`AdminUpdateUserAttributes`, `phone_number = +91<corp mobile>` + `phone_number_verified = "True"` — mirrors auth-backend's own `updateCognitoUserPhoneNumber`; changes the number the user signs in with). Guards: refused when ANY Cognito user already holds the corp mobile (phone is a sign-in alias — matching short code → use the cognito_id fix; different user → correct that account first), and on lookup errors / ambiguous stored subs. The preview also searches corp + auth for employees holding the OLD Cognito mobile and lists them (warn-only, not blocking — blocking would deadlock, since freeing the number is exactly what unblocks the other user's fix); after the update, analyze the old mobile to bring that user in sync.
7. **Return account to its owner** — the stored sub's Cognito account holds THIS employee's corp mobile but its `custom:emp_short_code` identifies a DIFFERENT employee. The short code is the identity anchor and is NEVER rewritten (user-directed 2026-07-13: the only permitted Cognito write is the mobile number) — per corp, the account belongs to the OTHER employee, and one run repairs both users: (1) the account's Cognito `phone_number` is set to the OWNER's corp mobile via `updateUserPhone` (skipped when already equal); (2) "based on the short code in Cognito" the sub is written to the owner's corp `empmaster_hdr` + auth `Field_Force_Users` cognito_id (only sides that differ); (3) the stale link on THIS employee's corp/auth rows is NULLed via `releaseSubFromRecords` (conditional `AND cognito_id = <sub>`). Run order phone → owner writes → clear keeps every mid-run failure re-runnable. Guards: exactly ONE corp employee may carry the account's short code (0 or >1 → refuse; analyze the owner's mobile instead); the owner's corp/auth rows must not store a DIFFERENT cognito_id (could be their real newer account); no other Cognito account may already hold the owner's corp mobile; the sub must not sit on any third-party record (neither this employee's nor the owner's); refused on lookup errors or when a live mobile+short-code target exists for this employee (fix applies). A phone-matching mismatched-shortcode owner is never "wrong"/clearable by the stored-sub audit — this reassign is offered instead of Clear. The preview shows the owner's identity, the phone before→after, the owner-side cognito_id writes, and the records being cleared; afterwards re-check this employee and analyze the owner's mobile to verify.
