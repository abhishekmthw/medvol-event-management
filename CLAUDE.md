...
| `lib/comparison-csv.ts` | Client-safe (type-only imports) CSV builder. `toComparisonCsv(result)` flattens each record to one column per source per field (auth/corp/cognito) + mismatch flags + status, with a UTF-8 BOM for Excel. Used by the "Download CSV" button on the results card. |
...
- **Why join in Node, not SQL:** auth and corp are reached through separately-configured pools that may point at different databases, so a cross-DB SQL join is unsafe. Each set is fetched independently and joined in application code by the **(short code, company code)** pair (`auth.short_code`+`auth.company_code` = `corp.emp_shortcode`+`corp.company_code`). A short code is only unique *within a company* (the same code is reused across companies), so company code is part of the match key. Both `company_code` columns hold the same human company code (auth `Companies.id` is a `@PrimaryColumn` = the code, and `Field_Force_Users.company_code` is the FK storing it; corp `empmaster_hdr.company_code` matches `company_hdr.code`), so they compare directly with no Companies lookup. Volume (field-force employees) is small enough for an internal tool.
- **Compared fields:** employee **name** (`name` / `emp_name`), **mobile number** (normalized to last 10 digits — auth `mobile_no` is varchar, corp `mobile_no` is `numeric(10)`, Cognito stores `+91…`), and **cognito id** (`cognito_id` vs the live Cognito `sub`). **Short code** and **company code** form the identity/match key (shown but not flagged — a difference means the records don't pair, surfacing as "Missing in auth/corp").
...
- **Two modes:**
  ...
  - **Mobile blank** → bulk scan: fetch all (scoped) employees from both DBs, join by the (short code, company code) pair, flag a record inconsistent if it exists in only one DB **or** name/mobile/cognito_id differ ("any difference"), take the **first 100** (deterministic, sorted by the composite key), then enrich those with Cognito.
...
- **CSV export:** the results card has a **Download CSV** button. The CSV is built client-side from the already-loaded result (`lib/comparison-csv.ts`, no extra request) and flattens each record into side-by-side auth/corp/cognito columns per field plus mismatch flags and the status notes. Filename encodes env / scope / mode / mobile / timestamp.
