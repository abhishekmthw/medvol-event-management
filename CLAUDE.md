...
| `app/api/auth-comparison/correction/sync-cognito-attrs/route.ts` | `POST {environment, empmasterId, preview}` — overwrites the Cognito user's stale `custom:emp_short_code` / `name` / `custom:ucode` from corp when the account holds the corp mobile but identifies a different employee (`preview: true` = attribute before/after + claimed-by records only). |
